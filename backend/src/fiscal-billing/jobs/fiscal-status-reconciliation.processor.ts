...
export class FiscalStatusReconciliationProcessor implements OnModuleInit{
  ...
  onModuleInit(){this.workers.registerWorker(FISCAL_STATUS_RECONCILIATION_WORKER_KEY,PLATFORM_QUEUE_KEYS.FISCAL_STATUS_RECONCILIATION,job=>this.process(job as Job<JobEnvelope<unknown>>),{concurrency:FISCAL_STATUS_RECONCILIATION_CONCURRENCY,jobNames:FISCAL_STATUS_RECONCILIATION_JOB_NAME});}
...
function invalid():never{throw new UnrecoverableError("FISCAL_STATUS_RECONCILIATION_JOB_INVALID");}
