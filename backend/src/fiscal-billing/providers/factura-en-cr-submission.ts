...
export interface FacturaEnCrSubmissionAggregate {
  id: string; tenantId: string; documentTypeCode: string; fiscalCalculationPolicyVersion: string | null; issuerIdentification: string; issuerEconomicActivityCode: string | null;
...
export function prepareFacturaEnCrSubmission(d: FacturaEnCrSubmissionAggregate): FacturaEnCrPreparedSubmission {
  const calculatedPolicy = d.fiscalCalculationPolicyVersion === CR_V44_DECIMAL_V1;
  if (d.fiscalCalculationPolicyVersion !== null && !calculatedPolicy) fail("FACTURA_EN_CR_CALCULATION_POLICY_UNSUPPORTED");
  if (calculatedPolicy) {
...
function mapLine(l:FacturaEnCrLineSnapshot,calculatedPolicy:boolean):Json {
  ...
  const q=decimal(l.quantity,calculatedPolicy?16:18,calculatedPolicy?3:4,true),price=decimal(l.unitPrice,18,calculatedPolicy?5:4,false),gross=decimal(l.grossAmount,18,calculatedPolicy?5:4,false),discount=decimal(l.discountAmount,18,calculatedPolicy?5:4,false),base=decimal(l.taxableBase,18,calculatedPolicy?5:4,false),taxTotal=decimal(l.taxAmount,18,calculatedPolicy?5:4,false),exemptedTotal=decimal(l.exoneratedTaxAmount,18,calculatedPolicy?5:4,false),netTotal=decimal(l.netTaxAmount,18,calculatedPolicy?5:4,false),subtotal=decimal(l.lineSubtotal,18,calculatedPolicy?5:4,false),total=decimal(l.lineTotal,18,calculatedPolicy?5:4,false);
  if(!calculatedPolicy&&(!eq(mul(q,price),gross)||!eq(sub(gross,discount),subtotal)||!eq(base,subtotal)))fail("FACTURA_EN_CR_LINE_TAX_INVALID");
  ...
  const mapped=taxes.map(t=>{const x=mapTax(t,calculatedPolicy);if(!eq(x.base,base))fail("FACTURA_EN_CR_LINE_TAX_INVALID");taxSum=add(taxSum,x.amount);exSum=add(exSum,x.exempted);netSum=add(netSum,x.net);return x.json;});if(!eq(taxSum,taxTotal)||!eq(exSum,exemptedTotal)||!eq(netSum,netTotal)||!eq(add(subtotal,netTotal),total))fail("FACTURA_EN_CR_LINE_TAX_INVALID");
...
function mapTax(t:FacturaEnCrTaxSnapshot,calculatedPolicy:boolean):{json:Json;base:Dec;amount:Dec;exempted:Dec;net:Dec}{
  if(t.taxCode!=="01"||!(t.rateCode in IVA_RATES))fail("FACTURA_EN_CR_LINE_TAX_INVALID");const rate=decimal(t.ratePercentage,calculatedPolicy?5:7,calculatedPolicy?2:4,false);if(rate.canonical!==IVA_RATES[t.rateCode]||t.calculationFactor!==null)fail("FACTURA_EN_CR_LINE_TAX_INVALID");const base=decimal(t.taxableBase,18,calculatedPolicy?5:4,false);const amount=decimal(t.taxAmount,18,calculatedPolicy?5:4,false),net=decimal(t.netTaxAmount,18,calculatedPolicy?5:4,false);if(!calculatedPolicy&&!eq(percent(base,rate),amount))fail("FACTURA_EN_CR_LINE_TAX_INVALID");let exempted=zero();const json:{[key:string]:Json}={codigo:t.taxCode,codigoTarifa:t.rateCode,tarifa:new ExactDecimal(rate.canonical)};if(t.exemption){const x=mapExemption(t.exemption);json.exoneracion=x.json;exempted=x.amount;}if(!eq(sub(amount,exempted),net))fail("FACTURA_EN_CR_LINE_TAX_INVALID");return{json,base,amount,exempted,net};
}
