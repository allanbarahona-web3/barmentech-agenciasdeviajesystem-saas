...
describe("BillingDocumentStatusPersistenceService",()=>{
  it.each(["queued","future_safe_status"])("keeps %s non-final processing idempotently without a write or clock",async providerStatus=>{
    const c=context(row(),lookup({providerResult:{providerStatus}}));
    const result=await c.service.persist(c.lookup);
    expect(result).toEqual({tenantId:"tenant-a",billingDocumentId:"document-a",final:false,finalDecision:null,lifecycleStatus:"SUBMITTED",providerStatus:"PROCESSED",taxAuthorityStatus:"PROCESSING",issuedAt:null,newlyPersisted:false,rejectionDetail:null});
    ...
    expect(c.tx.billingDocument.updateMany).not.toHaveBeenCalled();expect(c.clock.now).not.toHaveBeenCalled();noSideEffects(c);
  });
  ...
  it("atomically persists acceptance with one clock reading and exact tenant-scoped CAS",async()=>{
    ...
    expect(write.data).toEqual({taxAuthorityStatus:"ACCEPTED",providerReconciliationRequired:false,providerLastErrorCode:null,providerLastErrorAt:null,issuedAt:ISSUED});
  ...
  it("returns an exact accepted winner idempotently and preserves its original issuedAt",async()=>{
    const c=context(row({taxAuthorityStatus:"ACCEPTED",issuedAt:ISSUED}),lookup({providerResult:accepted()}));
    const result=await c.service.persist(c.lookup);
  ...
  it("persists rejection without issuedAt or free-text/error persistence",async()=>{
    ...
    expect(write.data).toEqual({taxAuthorityStatus:"REJECTED",providerReconciliationRequired:false,providerLastErrorCode:null,providerLastErrorAt:null});
    expect(JSON.stringify(write.data)).not.toContain(detail);expect(c.clock.now).not.toHaveBeenCalled();noSideEffects(c);
  });
  ...
  it("returns a normalized rejection detail longer than 1,000 characters unchanged and never persists it",async()=>{
    ...
    expect(JSON.stringify(write.data)).not.toContain(detail);expect(write.data).not.toHaveProperty("rejectionDetail");expect(c.clock.now).not.toHaveBeenCalled();
  });
  ...
  it("returns an exact rejected state idempotently",async()=>{
    const input=lookup({persistedIdentity:{taxAuthorityStatus:"REJECTED"},providerResult:rejected("detail")});
    const c=context(row({taxAuthorityStatus:"REJECTED"}),input);const result=await c.service.persist(input);
  ...
  it("never downgrades accepted or changes rejected to accepted",async()=>{
    let c=context(row({taxAuthorityStatus:"ACCEPTED",issuedAt:ISSUED}),lookup());
    ...
    c=context(row({taxAuthorityStatus:"REJECTED"}),lookup({providerResult:accepted()}));
  ...
  it("recognizes accepted and rejected winners in the authoritative locked reread without clock or write",async()=>{
    for(const decision of ["ACCEPTED","REJECTED"] as const){const finalRow=decision==="ACCEPTED"?row({taxAuthorityStatus:"ACCEPTED",issuedAt:ISSUED}):row({taxAuthorityStatus:"REJECTED"});
      const input=lookup({providerResult:decision==="ACCEPTED"?accepted():rejected(null)});const c=context(finalRow,input);
  ...
  it("treats an accepted zero-row CAS after its single clock read as conflict, never idempotent success",async()=>{
    ...
    expect(c.clock.now).toHaveBeenCalledTimes(1);expect(c.tx.billingDocument.findUnique).toHaveBeenCalledTimes(1);
  });
  ...
  it("can defensively recognize an exact rejected winner after a zero-row CAS without clock",async()=>{
    ...
    c.tx.billingDocument.findUnique.mockResolvedValueOnce(row()).mockResolvedValueOnce(row({taxAuthorityStatus:"REJECTED"}));
    await expect(c.service.persist(input)).resolves.toMatchObject({finalDecision:"REJECTED",newlyPersisted:false});expect(c.clock.now).not.toHaveBeenCalled();
  });
  ...
  it("compares the supported BigInt upper boundary without numeric conversion",async()=>{
    ...
    await expect(c.service.persist(input)).resolves.toMatchObject({newlyPersisted:false});
...
function row(overrides:Record<string,unknown>={}){return{id:"document-a",tenantId:"tenant-a",billingMode:"ELECTRONIC_PROVIDER",lifecycleStatus:"SUBMITTED",providerStatus:"PROCESSED",taxAuthorityStatus:"PROCESSING",
  ...
  providerLastAttemptAt:ATTEMPT,providerReconciliationRequired:false,providerLastErrorCode:null,providerLastErrorAt:null,submittedAt:ATTEMPT,issuedAt:null,...overrides};}
function lookup(overrides:{persistedIdentity?:Record<string,unknown>;providerResult?:Record<string,unknown>}={}):BillingDocumentStatusLookupResult{return{persistedIdentity:{tenantId:"tenant-a",billingDocumentId:"document-a",billingDocumentNumberSequenceId:"sequence-a",allocatedSequenceNumber:"42",providerDocumentId:"provider_a-1",haciendaKey:KEY,
  ...
  fiscalIssueDate:"2026-08-24",lifecycleStatus:"SUBMITTED",providerStatus:"PROCESSED",taxAuthorityStatus:"PROCESSING",providerReconciliationRequired:false,submittedAt:ATTEMPT,issuedAt:null,...overrides.persistedIdentity},
