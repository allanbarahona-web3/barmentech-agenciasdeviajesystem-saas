...
import { FISCAL_INVOICE_AUTO_DELIVERY_REQUESTED_EVENT_TYPE } from "./jobs/fiscal-accepted-fanout.constants";
...
function context(overrides: { artifacts?: Array<{ artifactType: string; status: string }>; document?: Record<string, unknown>; emailResult?: Record<string, unknown> } = {}) {
  const child = { id: "child-a", tenantId: "tenant-a", eventType: FISCAL_INVOICE_AUTO_DELIVERY_REQUESTED_EVENT_TYPE, eventVersion: 1, aggregateType: "BillingDocument", aggregateId: "document-a", causationId: "parent-a", payload: { tenantId: "tenant-a", billingDocumentId: "document-a", eventVersion: 1 }, attemptCount: 1, maximumAttempts: 5 };
  ...
  const prisma = { $transaction: jest.fn(async (work: (client: typeof tx) => unknown) => work(tx)) } as unknown as PrismaService;
  ...
  return { service: new FiscalInvoiceAutoDeliveryService(prisma, { generateAndPersist: pdf } as unknown as FiscalInvoicePdfService, { download } as unknown as FiscalArtifactReadService, { sendEmail: send } as unknown as EmailService), prisma, tx, pdf, download, send, audit, update };
}
