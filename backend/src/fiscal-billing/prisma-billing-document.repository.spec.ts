...
describe("PrismaBillingDocumentRepository generic draft persistence", () => {
  ...
  it.each([[9999999999n,"9999999999"],[null,null]] as const)("returns distinct tenant-scoped fiscal lifecycle timestamps",async(allocated,expected)=>{const findUnique=jest.fn().mockResolvedValue(workspaceRow({allocatedSequenceNumber:allocated}));const repository=new PrismaBillingDocumentRepository({billingDocument:{findUnique}} as never);const result=await repository.findWorkspace("tenant-a","document-a");expect(findUnique).toHaveBeenCalledWith(expect.objectContaining({where:{id_tenantId:{id:"document-a",tenantId:"tenant-a"}}}));expect(result?.allocatedSequenceNumber).toBe(expected);expect(result?.fiscalCalculationPolicyVersion).toBeNull();expect(result?.exchangeRate).toBe("454.340000000001");expect(result?.fiscalIssueDate).toBe("2026-08-24");expect(result?.fiscalEmissionAt?.getTime()).toBe(new Date("2026-08-24T12:00:00.123Z").getTime());expect(result?.submittedAt?.getTime()).toBe(new Date("2026-08-24T12:00:00.123Z").getTime());expect(result?.issuedAt).toBeNull();expect(result?.taxAuthorityFinalizedAt?.getTime()).toBe(new Date("2026-08-24T12:01:00.123Z").getTime());expect(result?.submittedAt).not.toBe((await findUnique.mock.results[0].value)?.submittedAt);expect(result?.haciendaRejectionDetail).toBe("Normalized Hacienda detail");expectNoBigInt(result);const select=findUnique.mock.calls[0][0].select;expect(select).toHaveProperty("taxAuthorityFinalizedAt",true);for(const internal of ["tenantId","providerRequestHash","issuanceIdempotencyKey","billingDocumentNumberSequenceId","providerStatusCheckLockOwner","providerStatusCheckLeaseUntil","providerRefreshLockOwner","providerRefreshLeaseUntil"])expect(select).not.toHaveProperty(internal);});

  it("marks an exact CR policy draft ready without a draft-time exchange rate",async()=>{
    const findUnique=jest.fn().mockResolvedValue(readyWorkspaceRow({currencyCode:"USD"}));
    const result=await new PrismaBillingDocumentRepository({billingDocument:{findUnique}} as never).findWorkspace("tenant-a","document-a");
    expect(result?.readiness).toEqual({receiverFiscalIdentityMissing:false,exchangeRateMissing:false,fiscalCalculationPolicyUnsupported:false,calculatedSnapshotInvalid:false,issuanceReady:true,issues:[]});
    expect(findUnique).toHaveBeenCalledTimes(1);
  });

  it.each([null,""," ","UNKNOWN_POLICY"])("keeps readable policy %p drafts not issuance-ready",async fiscalCalculationPolicyVersion=>{
    const result=await new PrismaBillingDocumentRepository({billingDocument:{findUnique:jest.fn().mockResolvedValue(readyWorkspaceRow({fiscalCalculationPolicyVersion}))}} as never).findWorkspace("tenant-a","document-a");
    expect(result?.fiscalCalculationPolicyVersion).toBe(fiscalCalculationPolicyVersion);expect(result?.readiness.issuanceReady).toBe(false);expect(result?.readiness.fiscalCalculationPolicyUnsupported).toBe(true);expect(result?.readiness.issues).toContain("BILLING_DOCUMENT_FISCAL_CALCULATION_POLICY_UNSUPPORTED");
  });

  it("marks a contradictory calculated snapshot invalid without modifying it",async()=>{
    const persisted=readyWorkspaceRow({total:new Prisma.Decimal("35.31")});
    const result=await new PrismaBillingDocumentRepository({billingDocument:{findUnique:jest.fn().mockResolvedValue(persisted)}} as never).findWorkspace("tenant-a","document-a");
    expect(result?.total).toBe("35.31");expect(result?.readiness.issuanceReady).toBe(false);expect(result?.readiness.calculatedSnapshotInvalid).toBe(true);expect(result?.readiness.issues).toContain("BILLING_DOCUMENT_CALCULATED_SNAPSHOT_INVALID");
  });
