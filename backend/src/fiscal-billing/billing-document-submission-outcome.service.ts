...
const outcomeSelect=Prisma.validator<Prisma.BillingDocumentSelect>()({id:true,tenantId:true,billingMode:true,lifecycleStatus:true,providerStatus:true,taxAuthorityStatus:true,
  ...
  providerLastErrorCode:true,providerLastErrorAt:true,providerReconciliationRequired:true,providerDocumentId:true,haciendaKey:true,haciendaRejectionDetail:true,providerEnvironment:true,fiscalEmissionAt:true,submittedAt:true,issuedAt:true,taxAuthorityFinalizedAt:true,
  ...statusCheckSelect()} as Prisma.BillingDocumentSelect);
...
type OutcomeRow=Prisma.BillingDocumentGetPayload<{select:typeof outcomeSelect}>&StatusCheckFields;
...
export class BillingDocumentSubmissionOutcomeService{
  ...
  async persist(outcome:BillingDocumentSubmissionExecutorOutcome):Promise<BillingDocumentSubmissionOutcomePersistenceResult>{
    ...
    try{
      return await this.prisma.$transaction(async tx=>{
        ...
        const row=await tx.billingDocument.findUnique({where:{id_tenantId:{id:identity.billingDocumentId,tenantId:identity.tenantId}},select:outcomeSelect}) as OutcomeRow|null;if(!row)notFound();
  ...
  private async persistAcknowledgement(tx:Prisma.TransactionClient,row:OutcomeRow,outcome:Extract<BillingDocumentSubmissionExecutorOutcome,{classification:"ACKNOWLEDGED"}>,identity:Identity){
    ...
    const haciendaRejectionDetail=taxAuthorityStatus==="REJECTED"?outcome.acknowledgement.rejectionDetail:null;
    const scheduling=taxAuthorityStatus==="PROCESSING"?initialFiscalStatusReconciliationSchedule(outcome.attempt.attemptedAt):{nextStatusCheckAt:null,reconciliationRequired:false};const terminal=taxAuthorityStatus==="ACCEPTED"||taxAuthorityStatus==="REJECTED";const finalizedAt=terminal?this.clock.now():null;if(terminal&&!validDate(finalizedAt))corrupt();
    const updated=await tx.billingDocument.updateMany({where:claimedWhere(row,outcome.attempt),data:{providerDocumentId:outcome.acknowledgement.providerDocumentId,
      ...
      haciendaRejectionDetail,lifecycleStatus:"SUBMITTED",submittedAt:outcome.attempt.attemptedAt,providerReconciliationRequired:scheduling.reconciliationRequired,providerLastErrorCode:null,providerLastErrorAt:null,
      providerStatusCheckAttempts:0,providerLastStatusCheckAt:null,providerNextStatusCheckAt:scheduling.nextStatusCheckAt,providerStatusCheckLockOwner:null,providerStatusCheckLeaseUntil:null,
  ...
  private async persistError(tx:Prisma.TransactionClient,row:OutcomeRow,code:string,reconciliation:boolean,identity:Identity){
    ...
    const updated=await tx.billingDocument.updateMany({where:claimedWhere(row,{tenantId:identity.tenantId,billingDocumentId:identity.billingDocumentId,requestHash:row.providerRequestHash!,attemptedAt:row.providerLastAttemptAt!}),
      data:{haciendaRejectionDetail:null,providerLastErrorCode:code,providerLastErrorAt:errorAt,providerReconciliationRequired:reconciliation,providerStatusCheckAttempts:0,providerLastStatusCheckAt:null,providerNextStatusCheckAt:null,providerStatusCheckLockOwner:null,providerStatusCheckLeaseUntil:null,...(!reconciliation?{providerStatus:"FAILED" as const}:{})} as Prisma.BillingDocumentUpdateManyMutationInput});
    if(updated.count!==1)outcomeConflict();return persisted(identity);
  }
  private async recoverAcknowledgementWinner(outcome:Extract<BillingDocumentSubmissionExecutorOutcome,{classification:"ACKNOWLEDGED"}>,identity:Identity){
    let row:OutcomeRow|null;try{row=await this.prisma.billingDocument.findUnique({where:{id_tenantId:{id:identity.billingDocumentId,tenantId:identity.tenantId}},select:outcomeSelect}) as OutcomeRow|null;}catch{throw fiscalBillingError("BILLING_DOCUMENT_SUBMISSION_OUTCOME_PERSISTENCE_FAILED");}
...
function requireAllocated(row:OutcomeRow){if(row.billingMode!=="ELECTRONIC_PROVIDER"||!row.billingDocumentNumberSequenceId||row.allocatedSequenceNumber===null||!row.fiscalNumber||!row.issuanceIdempotencyKey||!row.fiscalIssueDate||!validDate(row.fiscalEmissionAt))corrupt();}
...
function requireClaimed(row:OutcomeRow){if(row.lifecycleStatus!=="CONFIRMED"||row.providerStatus!=="PENDING"||row.taxAuthorityStatus!=="NOT_SUBMITTED"||!row.providerReconciliationRequired||!row.providerRequestHash||!row.providerLastAttemptAt||row.providerDocumentId||row.haciendaKey||row.haciendaRejectionDetail||row.providerEnvironment||row.providerLastErrorCode||row.providerLastErrorAt||row.submittedAt||row.issuedAt||row.taxAuthorityFinalizedAt||!unscheduled(row))corrupt();}
function validateAcknowledgement(outcome:Extract<BillingDocumentSubmissionExecutorOutcome,{classification:"ACKNOWLEDGED"}>,row:OutcomeRow){const a=outcome.acknowledgement,s=a.status;if(a.classification!=="ACKNOWLEDGED_PROVIDER_SUBMISSION"||typeof a.providerDocumentId!=="string"||!a.providerDocumentId.trim()||a.providerDocumentId.length>255||!/^[0-9]{50}$/.test(a.haciendaKey)||a.consecutive!==row.fiscalNumber||a.haciendaKey.slice(21,41)!==row.fiscalNumber||(a.providerEnvironment!=="sandbox"&&a.providerEnvironment!=="production")||!/^[a-z][a-z0-9_]{0,63}$/.test(s.providerStatus)||s.accepted!==(s.providerStatus==="accepted")||s.rejected!==(s.providerStatus==="rejected")||s.final!==(s.accepted||s.rejected)||!normalizedRejectionDetail(a.rejectionDetail,s.rejected)||!keyDate(a.haciendaKey,row.fiscalIssueDate!))ackMismatch();}
...
function claimedWhere(row:OutcomeRow,attempt:Attempt){return{id:attempt.billingDocumentId,tenantId:attempt.tenantId,billingMode:"ELECTRONIC_PROVIDER" as const,lifecycleStatus:"CONFIRMED" as const,providerStatus:"PENDING" as const,taxAuthorityStatus:"NOT_SUBMITTED" as const,
  ...
  providerRequestHash:attempt.requestHash,providerLastAttemptAt:attempt.attemptedAt,providerReconciliationRequired:true,providerDocumentId:null,haciendaKey:null,haciendaRejectionDetail:null,providerEnvironment:null,providerLastErrorCode:null,providerLastErrorAt:null,submittedAt:null,issuedAt:null,taxAuthorityFinalizedAt:null,
  providerStatusCheckAttempts:0,providerLastStatusCheckAt:null,providerNextStatusCheckAt:null,providerStatusCheckLockOwner:null,providerStatusCheckLeaseUntil:null} as Prisma.BillingDocumentWhereInput;}
function completeAcknowledgement(row:OutcomeRow){return row.lifecycleStatus==="SUBMITTED"&&row.providerStatus==="PROCESSED"&&row.taxAuthorityStatus!=="NOT_SUBMITTED"&&normalizedRejectionDetail(row.haciendaRejectionDetail,row.taxAuthorityStatus==="REJECTED")&&!!row.providerRequestHash&&!!row.providerLastAttemptAt&&!row.providerReconciliationRequired&&!row.providerLastErrorCode&&!row.providerLastErrorAt&&typeof row.providerDocumentId==="string"&&!!row.providerDocumentId.trim()&&row.providerDocumentId.length<=255&&
  typeof row.haciendaKey==="string"&&/^\d{50}$/.test(row.haciendaKey)&&row.haciendaKey.slice(21,41)===row.fiscalNumber&&!!row.fiscalIssueDate&&keyDate(row.haciendaKey,row.fiscalIssueDate)&&(row.providerEnvironment==="sandbox"||row.providerEnvironment==="production")&&!!row.submittedAt&&initialScheduleComplete(row)&&(row.taxAuthorityStatus==="PROCESSING"?row.issuedAt===null&&row.taxAuthorityFinalizedAt===null:validDate(row.taxAuthorityFinalizedAt)&&(row.taxAuthorityStatus==="ACCEPTED"?row.issuedAt?.getTime()===row.fiscalEmissionAt?.getTime():row.issuedAt===null));}
function completeFailure(row:OutcomeRow){return row.lifecycleStatus==="CONFIRMED"&&row.providerStatus==="FAILED"&&row.taxAuthorityStatus==="NOT_SUBMITTED"&&row.haciendaRejectionDetail===null&&!row.providerReconciliationRequired&&!!row.providerRequestHash&&!!row.providerLastAttemptAt&&typeof row.providerLastErrorCode==="string"&&/^[A-Z][A-Z0-9_]{0,99}$/.test(row.providerLastErrorCode)&&!!row.providerLastErrorAt&&!row.providerDocumentId&&!row.haciendaKey&&!row.providerEnvironment&&!row.submittedAt&&!row.issuedAt&&!row.taxAuthorityFinalizedAt&&unscheduled(row);}
function completeReconciliation(row:OutcomeRow){return row.lifecycleStatus==="CONFIRMED"&&row.providerStatus==="PENDING"&&row.taxAuthorityStatus==="NOT_SUBMITTED"&&row.haciendaRejectionDetail===null&&row.providerReconciliationRequired&&!!row.providerRequestHash&&!!row.providerLastAttemptAt&&!row.providerDocumentId&&!row.haciendaKey&&!row.providerEnvironment&&!row.submittedAt&&!row.issuedAt&&!row.taxAuthorityFinalizedAt&&(row.providerLastErrorCode===null)===(row.providerLastErrorAt===null)&&(row.providerLastErrorCode===null||/^[A-Z][A-Z0-9_]{0,99}$/.test(row.providerLastErrorCode))&&unscheduled(row);}
function matchesAcknowledgement(row:OutcomeRow,outcome:Extract<BillingDocumentSubmissionExecutorOutcome,{classification:"ACKNOWLEDGED"}>){const a=outcome.acknowledgement,s=a.status,target=s.accepted?"ACCEPTED":s.rejected?"REJECTED":"PROCESSING";const expected=target==="PROCESSING"?initialFiscalStatusReconciliationSchedule(outcome.attempt.attemptedAt).nextStatusCheckAt:null;return row.id===outcome.attempt.billingDocumentId&&row.tenantId===outcome.attempt.tenantId&&row.billingMode==="ELECTRONIC_PROVIDER"&&!!row.billingDocumentNumberSequenceId&&row.allocatedSequenceNumber!==null&&!!row.fiscalNumber&&!!row.issuanceIdempotencyKey&&completeAcknowledgement(row)&&row.providerRequestHash===outcome.attempt.requestHash&&row.providerLastAttemptAt?.getTime()===outcome.attempt.attemptedAt.getTime()&&row.submittedAt?.getTime()===outcome.attempt.attemptedAt.getTime()&&(target==="ACCEPTED"?row.issuedAt?.getTime()===row.fiscalEmissionAt?.getTime():row.issuedAt===null)&&row.providerDocumentId===a.providerDocumentId&&row.haciendaKey===a.haciendaKey&&row.haciendaRejectionDetail===(target==="REJECTED"?a.rejectionDetail:null)&&row.providerEnvironment===a.providerEnvironment&&row.taxAuthorityStatus===target&&row.providerStatusCheckAttempts===0&&row.providerLastStatusCheckAt===null&&dateTime(row.providerNextStatusCheckAt)===dateTime(expected)&&row.providerStatusCheckLockOwner===null&&row.providerStatusCheckLeaseUntil===null;}
function normalizedRejectionDetail(value:unknown,rejected:boolean):boolean{return value===null?true:rejected&&typeof value==="string"&&value.length>=1&&value.length<=65_536&&value===value.trim()&&!/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(value);}
function unscheduled(row:StatusCheckFields){return row.providerStatusCheckAttempts===0&&row.providerLastStatusCheckAt===null&&row.providerNextStatusCheckAt===null&&row.providerStatusCheckLockOwner===null&&row.providerStatusCheckLeaseUntil===null;}
function dateTime(value:Date|null){return value?.getTime()??null;}
function validDate(value:unknown):value is Date{return value instanceof Date&&Number.isFinite(value.getTime());}
function initialScheduleComplete(row:OutcomeRow){if(row.providerStatusCheckAttempts!==0||row.providerLastStatusCheckAt!==null||row.providerStatusCheckLockOwner!==null||row.providerStatusCheckLeaseUntil!==null)return false;if(row.taxAuthorityStatus!=="PROCESSING")return row.providerNextStatusCheckAt===null;return dateTime(row.providerNextStatusCheckAt)===dateTime(initialFiscalStatusReconciliationSchedule(row.submittedAt!).nextStatusCheckAt);}
