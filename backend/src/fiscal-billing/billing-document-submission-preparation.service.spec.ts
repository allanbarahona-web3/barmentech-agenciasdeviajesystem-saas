...
describe("BillingDocumentSubmissionPreparationService",()=>{
  ...
  it("loads one tenant-scoped bounded aggregate, maps exact snapshots, and returns the builder result unchanged",async()=>{
    ...
    expect(aggregate.allocatedSequenceNumber).toBe("9007199254740993123");
  ...
  it("fails safely for incomplete eligibility, builder rejection, and Prisma read errors",async()=>{
    const incomplete=row({allocatedSequenceNumber:null});
...
function row(overrides:Record<string,unknown>={}){return{id:"document-a",tenantId:"tenant-a",documentTypeCode:"01",billingMode:"ELECTRONIC_PROVIDER",lifecycleStatus:"CONFIRMED",
  ...
  billingDocumentNumberSequenceId:"sequence-a",allocatedSequenceNumber:9007199254740993123n,fiscalNumber:"00100001010000000042",issuanceIdempotencyKey:"billing-document:document-a:electronic-issuance:v1",
...
function prepared(){return{endpoint:"/documents/factura" as const,canonicalBody:'{"safe":true}',requestHash:"a".repeat(64),idempotencyKey:"billing-document:document-a:electronic-issuance:v1",metadata:{billingDocumentId:"document-a",tenantId:"tenant-a",documentTypeCode:"01" as const,fiscalNumber:"00100001010000000042",fiscalIssueDate:"2026-08-24"}};}
