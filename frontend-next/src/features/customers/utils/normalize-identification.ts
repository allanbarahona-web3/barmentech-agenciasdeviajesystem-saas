...
/**
 * Generates all possible normalized variations of a search term
 * for ID number searching when idType is unknown.
 * 
 * Used when searching for customers without knowing their ID type.
 * Returns an array of normalized values to search for.
 */
export function normalizeIdentificationForSearch(
  ...
  // If search term looks like it could be an ID number (contains digits)
  if (/\d/.test(raw)) {
    // Try normalizing as Cedula
    variations.push(normalizeIdentification("Cedula", raw));
    
    // Try normalizing as DIMEX
    variations.push(normalizeIdentification("DIMEX", raw));
  }

  // Always include the trimmed original for passport/text search
  variations.push(raw);
