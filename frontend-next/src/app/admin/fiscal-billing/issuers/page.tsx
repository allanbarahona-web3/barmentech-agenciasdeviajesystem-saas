...
const CR_IDENTIFICATION_GUIDANCE: Record<string, string> = {
  '01': '9 dígitos. Ejemplo: 1-1234-5678',
  '02': '10 dígitos. Ejemplo: 3-102-884562',
  '03': '11 o 12 dígitos.',
  '04': '10 dígitos.',
};

const IDENTIFICATION_INVALID_MESSAGE =
...
function validate(form: IssuerFormState) {
  ...
  if (country === 'CR') {
    ...
    if (!/^[0-9 -]+$/.test(identification)) {
      return IDENTIFICATION_INVALID_MESSAGE;
    }
    const canonical = identification.replace(/[ -]/g, '');
    const validLength =
      form.identificationTypeCode === '01'
        ? /^\d{9}$/.test(canonical)
        : form.identificationTypeCode === '02' ||
            form.identificationTypeCode === '04'
          ? /^\d{10}$/.test(canonical)
          : form.identificationTypeCode === '03'
            ? /^\d{11,12}$/.test(canonical)
            : false;
    if (!validLength) return IDENTIFICATION_INVALID_MESSAGE;
  }
