...
export function ReceivableGroupsView({ canWrite, reloadToken, onOpenDetail, onRegisterPayment, onViewPayments }: {
  ...
  return <section className={styles.tableCard}>
    ...
    {error ? <div className={styles.state}><div><span className={styles.stateIcon}><AlertCircle aria-hidden="true" /></span><h3 className={styles.error}>No se pudo cargar la cartera</h3><p>{error.message}</p><Button className={styles.secondaryAction} variant="outline" type="button" onClick={() => setRetry((value) => value + 1)}>Intentar nuevamente</Button></div></div> : !loading && (!result || result.groups.length === 0) ? <div className={styles.state}><div><span className={styles.stateIcon}><FileSearch aria-hidden="true" /></span><h3>No hay grupos de cuentas por cobrar</h3><p>Las deudas reconocidas aparecerán aquí después de la aceptación fiscal.</p></div></div> : <Table className={styles.groupTable}><TableHeader><TableRow><TableHead>Cliente / deudor</TableHead><TableHead>Moneda</TableHead><TableHead className={styles.numeric}>Original</TableHead><TableHead className={styles.numeric}>Aplicado</TableHead><TableHead className={styles.numeric}>Pendiente</TableHead><TableHead className={styles.numeric}>Vencido</TableHead><TableHead>Conteos</TableHead><TableHead>Acciones</TableHead></TableRow></TableHeader><TableBody>{loading ? Array.from({ length: 5 }, (_, row) => <TableRow key={row}>{Array.from({ length: 8 }, (_, cell) => <TableCell key={cell}><span className={styles.skeleton} /></TableCell>)}</TableRow>) : result?.groups.map((group) => <GroupRows key={group.groupKey} group={group} canWrite={canWrite} reloadToken={reloadToken} onOpenDetail={onOpenDetail} onRegisterPayment={onRegisterPayment} onViewPayments={onViewPayments} />)}</TableBody></Table>}
