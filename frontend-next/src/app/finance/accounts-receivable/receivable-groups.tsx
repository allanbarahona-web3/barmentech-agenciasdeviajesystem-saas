...
import { AlertCircle, ChevronDown, ChevronRight, CircleDollarSign, Eye, FilePlus2, PiggyBank, ReceiptText } from 'lucide-react';
...
function GroupRows({ group, canWrite, reloadToken, onOpenDetail, onRegisterPayment, onRegisterCustomerPayment, onApplyBalance, onApplyGroupBalance, onViewPayments, onStatement }: {
  group: AccountReceivableGroup;
  canWrite: boolean;
  reloadToken: number;
  onOpenDetail: (id: string) => void;
  onRegisterPayment: (id: string) => void;
  onRegisterCustomerPayment: (group: AccountReceivableGroup) => void;
          ...
          <div className={styles.childPanel}>
            ...
            {error ? <div className={styles.inlineError}><AlertCircle aria-hidden="true" /><span>{error}</span></div> : loading && !result ? <div className={styles.childLoading}>Cargando cuentas…</div> : result && result.accountReceivables.length > 0 ? <div className={styles.childTableWrap}>
              <Table className={styles.childTable}><TableHeader><TableRow><TableHead>Documento</TableHead><TableHead className={styles.numeric}>Monto original</TableHead><TableHead className={styles.numeric}>Saldo pendiente</TableHead><TableHead>Vencimiento</TableHead><TableHead>Estado</TableHead><TableHead>Acciones</TableHead></TableRow></TableHeader><TableBody>{result.accountReceivables.map((receivable) => <TableRow key={receivable.id}>
                ...
                <TableCell><div className={styles.rowActions}>{fiscalInvoiceHref(receivable.source) && <Button asChild className={styles.secondaryAction} size="sm" variant="outline"><Link href={fiscalInvoiceHref(receivable.source)!}><ReceiptText aria-hidden="true" />Ver factura</Link></Button>}<Button className={styles.secondaryAction} size="sm" type="button" variant="outline" onClick={() => onOpenDetail(receivable.id)}><Eye aria-hidden="true" />Detalle</Button>{canWrite && (receivable.status === 'SETTLED' || receivable.status === 'CANCELLED' ? <Button className={styles.secondaryAction} disabled size="sm" type="button" variant="outline" title="Esta cuenta ya no admite abonos.">No disponible</Button> : <><Button className={styles.actionButton} size="sm" type="button" onClick={() => onRegisterPayment(receivable.id)}><CircleDollarSign aria-hidden="true" />Registrar abono</Button>{group.unallocatedPaymentCount > 0 && <Button className={styles.creditAction} size="sm" type="button" onClick={() => onApplyBalance(receivable.id)}>Aplicar saldo</Button>}</>)}{fiscalInvoiceHref(receivable.source) && <DropdownMenu><DropdownMenuTrigger asChild><Button className={styles.secondaryAction} size="sm" type="button" variant="outline"><FilePlus2 aria-hidden="true" />Nota fiscal<ChevronDown aria-hidden="true" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled>Nota de crédito (NC) · Próximamente</DropdownMenuItem><DropdownMenuItem disabled>Nota de débito (ND) · Próximamente</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}</div></TableCell>
...
export function ReceivableGroupsView({ canWrite, reloadToken, onOpenDetail, onRegisterPayment, onRegisterCustomerPayment, onApplyBalance, onApplyGroupBalance, onViewPayments, onStatement }: {
  canWrite: boolean;
  reloadToken: number;
  onOpenDetail: (id: string) => void;
  onRegisterPayment: (id: string) => void;
