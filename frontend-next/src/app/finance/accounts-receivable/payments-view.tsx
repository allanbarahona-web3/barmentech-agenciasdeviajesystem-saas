...
export function PaymentsView({ canWrite, reloadToken, customerFilter, onClearCustomer, onChanged }: {
  ...
  }, [availableOnly, currency, customerFilter?.id, page, reloadToken, retry, status]);
  ...
  return <>
    <section className={styles.paymentFilters} aria-label="Filtros de pagos">
      ...
      <div className={styles.field}><label htmlFor="payment-currency">Moneda</label><select id="payment-currency" className={styles.select} value={currency} onChange={(event) => { setCurrency(event.target.value as FinanceCurrency | ''); setPage(1); }}><option value="">Todas</option><option value="CRC">CRC</option><option value="USD">USD</option></select></div>
