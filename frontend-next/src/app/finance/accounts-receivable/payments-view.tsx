...
function UnallocatedPaymentBalances({ canWrite, reloadToken, onApplyBalance }: { canWrite: boolean; reloadToken: number; onApplyBalance: (customer: PaymentCustomerFilter) => void }) {
  ...
  return <section className={`${styles.tableCard} ${styles.creditBalances}`}>
    <div className={styles.tableHeading}><div><h2>Saldo disponible — Recibos de dinero</h2><p>Dinero recibido sin aplicar, incluso cuando no existe una cuenta por cobrar para el cliente.</p></div><span>{loading ? 'Cargando…' : `${result?.total ?? 0} cliente(s) / moneda`}</span></div>
    {error ? <div className={styles.inlineError} role="alert"><AlertCircle aria-hidden="true" /><span>{error.message}</span><Button className={styles.secondaryAction} size="sm" type="button" variant="outline" onClick={() => setRetry((value) => value + 1)}>Intentar nuevamente</Button></div> : <Table className={styles.paymentTable}><TableHeader><TableRow><TableHead>Cliente / deudor</TableHead><TableHead>Moneda</TableHead><TableHead className={styles.numeric}>Saldo disponible</TableHead><TableHead className={styles.numeric}>Recibos con saldo</TableHead><TableHead>Acción</TableHead></TableRow></TableHeader><TableBody>{loading ? Array.from({ length: 3 }, (_, row) => <TableRow key={row}>{Array.from({ length: 5 }, (_, cell) => <TableCell key={cell}><span className={styles.skeleton} /></TableCell>)}</TableRow>) : result?.balances.map((balance) => <TableRow key={`${balance.customerId}-${balance.currencyCode}`}><TableCell><div className={styles.stack}><strong>{balance.debtor.displayName}</strong><span className={styles.secondary}>{balance.debtor.identificationNumber ?? balance.customerId}</span></div></TableCell><TableCell>{balance.currencyCode}</TableCell><TableCell className={styles.numeric}><strong>{formatFinanceMoney(balance.unallocatedPaymentAmount, balance.currencyCode)}</strong></TableCell><TableCell className={styles.numeric}>{balance.unallocatedPaymentCount}</TableCell><TableCell>{canWrite && <Button className={styles.secondaryAction} size="sm" type="button" variant="outline" onClick={() => onApplyBalance({ id: balance.customerId, name: balance.debtor.displayName, currency: balance.currencyCode })}>Ver recibos con saldo</Button>}</TableCell></TableRow>)}</TableBody></Table>}
